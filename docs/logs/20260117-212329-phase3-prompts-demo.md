...
ユーザーから Phase 3.4 以降の詳細実装計画を立てるよう依頼。
`docs/plans/20260117-212457-phase3.4-4-implementation.md` を作成。

### 2. デモモード追加

ユーザーから「Phase 3 完了時点で何ができるか」の質問。
各コンポーネントの動作確認ができるデモモードを計画に追加。

### 3. TDD 実装

- prompts/templates.py: フォーマット関数を追加
- main.py: `--demo search` と `--demo scrape` を実装

### 4. 動作確認

ユーザーが `--demo scrape` を実行したところ Playwright エラー発生。
`uv run playwright install chromium` で解決。
...
**テスト内容（13件）:**
- Planner: プレースホルダー確認、JSON指示確認、フォーマット関数、空タスクエラー（5件）
- Summarizer: プレースホルダー確認、フォーマット関数、max_length対応（3件）
- Reviewer: 両プレースホルダー確認、リスト結合（2件）
- Writer: 全プレースホルダー確認、参照フォーマット、空参照対応（3件）
...
### 3. Playwright ブラウザ未インストール

**発生箇所**: `--demo scrape` 実行時

**エラー内容**:
```
BrowserType.launch: Executable doesn't exist at ~/.cache/ms-playwright/chromium-1200/chrome-linux64/chrome
```

**原因**:
Crawl4AI が使用する Playwright の Chromium がインストールされていなかった。
...
docs/logs/20260117-212329.md    # 本ログ
```
...
```
コミット: aa97d0e Phase 3.4-3.5: プロンプトテンプレート＆デモモード実装（TDD）
プッシュ: da63313..aa97d0e main -> main
```
...
- **Playwright セットアップ**: Crawl4AI 使用時は `playwright install chromium` が必要

---

## 次回への引き継ぎ

- [ ] Phase 4 開始: `llm.py` の TDD 実装
- [ ] 各ノード（`nodes/*.py`）の TDD 実装
- [ ] デモモードに `--demo plan` と `--demo summarize` を追加（Phase 4 完了後）
